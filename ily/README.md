...
- `34-purr-private-reply-move-outcome-writeback-and-goldens.md` — closes the missing reply-planner calibration seam: hybrid typed outcome artifacts + ledger mirrors, explicit split between prediction/move/pack outcomes, deferred + heartbeat horizon closure, replay-safe writeback semantics for `response_value` / `timing_value`, and private reply-planner goldens so `feels one step ahead` gets sharper without prompt sludge
- `35-purr-reply-repair-boundary-and-outcome-hygiene.md` — closes the repaired-turn seam exposed by the latest Hermes read: one plan can span many execution attempts, `repair_outcome` stays separate from `move_outcome`, synthetic continuation/control text never becomes transcript evidence, and truncation/retry/rollback/compression handoff all get typed goldens instead of polluting relationship truth
- `36-purr-reply-execution-attempts-and-transcript-normalization-contract.md` — locks the durable publication boundary under streaming/tools/retry/resume: `reply_execution_attempt` vs `delivery_artifact` vs canonical `assistant_message_event`, idempotent finalization, tool-batch retry boundaries, restore-from-canonical-truth rules, and transcript-normalization goldens so runtime trace never masquerades as relationship history
- `37-purr-assistant-message-publication-and-surface-binding-contract.md` — closes the last outbound binding seam after note 36: `assistant_publication` between transcript truth and surface sends, server-owned `surface_binding`, per-surface `surface_delivery` rows, edit/replace/resend rules, media/voice wrapper normalization, notification/re-entry linkage, and cross-surface goldens so transport ids never become memory identity
